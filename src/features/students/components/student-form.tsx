...
const formSchema = z.object({
  pinfl: z
    .string()
    .min(6, { message: "PINFL kamida 6 ta belgidan iborat bo'lishi kerak" })
    .max(14, { message: 'PINFL 14 ta belgidan oshmasligi kerak' }),
  first_name: z
    .string()
  ...
  student_type: z.enum(['SCHOLARSHIP', 'CONTRACT'], {
    message: 'Talaba turi majburiy'
  }),
  course: z
    .number()
    .int()
    .min(1, { message: "Kurs kamida 1 bo'lishi kerak" })
    .max(2147483647, { message: 'Kurs haddan tashqari katta' }),
  until_date: z
    .string()
  ...
  image: z
    ...
    .refine((files) => !files || files?.length === 1, {
      message: 'Faqat bitta rasm yuklang'
    })
    .refine((files) => !files || files?.[0]?.size <= MAX_FILE_SIZE, {
      message: 'Maksimal fayl hajmi 5MB'
    })
    .refine(
      (files) => !files || ACCEPTED_IMAGE_TYPES.includes(files?.[0]?.type),
      { message: '.jpg, .jpeg, .png va .webp fayllar qabul qilinadi' }
    ),
  ...
  basis_document_file: z
    ...
    .optional()
    .refine((files) => !files || files?.length === 1, {
      message: 'Faqat bitta hujjat faylini yuklang'
    })
    .refine((files) => !files || files?.[0]?.size <= 10 * 1024 * 1024, {
      message: 'Maksimal fayl hajmi 10MB'
    })
    .refine(
      (files) => !files || ACCEPTED_DOCUMENT_TYPES.includes(files?.[0]?.type),
      { message: 'Faqat .pdf, .jpg, .png fayllar qabul qilinadi' }
    )
});
...
export default function StudentForm({
  ...
  const createStudent = useApiMutation<Student, FormData>('students/', 'POST', {
    onSuccess: () => {
  ...
  const updateStudent = useApiMutation<Student, FormData>(
  ...
  function onSubmit(values: FormData) {
    ...
    if (values.basis_document_number)
      formData.append('basis_document_number', values.basis_document_number);
    if (values.basis_document_file?.[0])
      formData.append('basis_document_file', values.basis_document_file[0]);
